...
import { getAllocationStartDateFromActivity, getScheduleIdFromActivity, getScheduleStartDateFromActivity, parseDate } from '../../../../utils/utils'
...
export default class AllocationDashboardRoutes {
  ...
  DEALLOCATE = async (req: Request, res: Response): Promise<void> => {
    ...
    if (allocationIds.length === 1 && parseIsoDate(getAllocationStartDateFromActivity(activity, +allocationIds[0])) > startOfToday()) {   res.redirect(
