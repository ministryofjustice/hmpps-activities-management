...
//
//       const { unlockListJourney } = req.session
//
//       expect(unlockListService.getFilteredUnlockList).toHaveBeenCalledWith(unlockListJourney, res.locals.user)
//       expect(res.render).toHaveBeenCalledWith('pages/activities/unlock-list/planned-events', {
//         unlockListJourney: unlockFilters,
//         unlockListItems,
//         movementCounts: {
//           leavingWing: 2,
//           stayingOnWing: 1,
...
//       const { unlockListJourney } = req.session
//
//       expect(unlockListJourney.locationFilters.includes({ value: 'A-Wing', text: 'A-Wing', checked: true }))
//
...
//       const { unlockListJourney } = req.session
//
//       expect(unlockListJourney).toBeUndefined()
...
//       expect(unlockListJourney.locationFilters.includes({ value: 'A-Wing', text: 'A-Wing', checked: true }))
//       expect(unlockListJourney.locationFilters.includes({ value: 'B-Wing', text: 'B-Wing', checked: false }))
//       expect(unlockListJourney.locationFilters.includes({ value: 'C-Wing', text: 'C-Wing', checked: true }))
//
//       expect(res.redirect).toHaveBeenCalledWith(
//         `planned-events?date=${formatDate(unlockListJourney.unlockDate, 'yyyy-MM-dd')}` +
//           `&slot=${unlockListJourney.timeSlot}&location=${unlockListJourney.location}`,
//       )
//     })
//
...
//       expect(unlockListJourney.activityFilters.includes({ value: 'Both', text: 'Both', checked: false }))
//       expect(unlockListJourney.activityFilters.includes({ value: 'With', text: 'With', checked: true }))
//       expect(unlockListJourney.activityFilters.includes({ value: 'Without', text: 'Without', checked: false }))
//
//       expect(res.redirect).toHaveBeenCalledWith(
//         `planned-events?date=${formatDate(unlockListJourney.unlockDate, 'yyyy-MM-dd')}` +
//           `&slot=${unlockListJourney.timeSlot}&location=${unlockListJourney.location}`,
...
//
//       // Different from the default filter values
//       expect(unlockListJourney.stayingOrLeavingFilters.includes({ value: 'Both', text: 'Both', checked: false }))
//       expect(unlockListJourney.stayingOrLeavingFilters.includes({ value: 'Staying', text: 'Staying', checked: true }))
//       expect(unlockListJourney.stayingOrLeavingFilters.includes({ value: 'Leaving', text: 'Leaving', checked: false }))
//
//       expect(res.redirect).toHaveBeenCalledWith(
//         `planned-events?date=${formatDate(unlockListJourney.unlockDate, 'yyyy-MM-dd')}` +
//           `&slot=${unlockListJourney.timeSlot}&location=${unlockListJourney.location}`,
...
//
//       const { unlockListJourney } = req.session
//       const { unlockDate, timeSlot, location } = unlockListJourney
//       const defaultFilters = testUnlockFilters()
//       expect(unlockListJourney).toEqual(defaultFilters)
//       expect(res.redirect).toHaveBeenCalledWith(
...
//
//       const { unlockListJourney } = req.session
//       expect(unlockListJourney).toBeDefined()
//       expect(unlockListJourney).toEqual(testUnlockFilters())
//
//       const { unlockDate, timeSlot, location } = unlockListJourney
//       expect(res.redirect).toHaveBeenCalledWith(
//         `planned-events?date=${formatDate(unlockDate, 'yyyy-MM-dd')}&slot=${timeSlot}&location=${location}`,
...
//       const { unlockListJourney } = req.session
//       expect(unlockListJourney).toBeDefined()
//       expect(unlockListJourney.locationFilters.filter(loc => loc.checked === true).map(loc => loc.value)).toEqual([
//         'B-Wing',
//         'C-Wing',
//       ])
//
...
//       expect(
//         unlockListJourney.activityFilters.filter(loc => loc.checked === true).filter(loc => loc.value === 'Both'),
//       ).toHaveLength(1)
//
//       const { unlockDate, timeSlot, location } = unlockListJourney
//       expect(res.redirect).toHaveBeenCalledWith(
