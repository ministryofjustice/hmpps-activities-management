...
describe('Route Handlers - Create Single Appointment - Category', () => {
  ...
  describe('GET', () => {
    it('should render the category view', async () => {
      ...
      expect(res.render).toHaveBeenCalledWith('pages/appointments/create-single/category', { categories })
    })
  })
  ...
  describe('POST', () => {
    it('should save selected category in session and redirect to check answers page', async () => {
      ...
      expect(req.session.createSingleAppointmentJourney.category).toEqual({
        id: 11,
        description: 'Medical - Doctor',
      })
