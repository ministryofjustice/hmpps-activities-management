...
export default class DateAndTimeRoutes {
  GET = async (req: Request, res: Response): Promise<void> => {
    res.render('pages/appointments/create/date-and-time')
  }

  POST = async (req: Request, res: Response): Promise<void> => {
    const { startDate, startTime, endTime } = req.body

    req.session.createAppointmentJourney.startDate = {
      day: startDate.day,
    ...
    req.session.createAppointmentJourney.startTime = {
      hour: startTime.hour,
      minute: startTime.minute,
      date: startTime.toDate(req.session.createAppointmentJourney.startDate.date),
    }

    req.session.createAppointmentJourney.endTime = {
      hour: endTime.hour,
      minute: endTime.minute,
      date: endTime.toDate(req.session.createAppointmentJourney.startDate.date),
    }
