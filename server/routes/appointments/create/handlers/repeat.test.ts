...
describe('Route Handlers - Create Appointment - Repeat', () => {
  ...
  describe('GET', () => {
    it('should render the repeat view', async () => {
      ...
      expect(res.render).toHaveBeenCalledWith('pages/appointments/create/repeat')
    })
  })
  ...
  describe('POST', () => {
    it('should save repeat = NO in session and redirect to check answers page', async () => {
      ...
      expect(req.session.createAppointmentJourney.repeat).toEqual(YesNo.NO)
    ...
    it('should save repeat = YES in session and redirect to repeat period and count page', async () => {
      ...
      expect(req.session.createAppointmentJourney.repeat).toEqual(YesNo.YES)
    ...
    it('should not save repeat = YES in session if previously was NO and redirect to repeat period and count page', async () => {
      req.session.createAppointmentJourney.repeat = YesNo.NO
      req.body = {
        repeat: YesNo.YES,
      }
    ...
    it('should redirect to repeat period and count page page when repeat = YES has not changed but repeat period is not set', async () => {
      req.session.createAppointmentJourney.repeat = YesNo.YES
      req.session.createAppointmentJourney.repeatPeriod = undefined
      req.session.createAppointmentJourney.repeatCount = 6
      req.body = {
        repeat: YesNo.YES,
      }
    ...
    it('should redirect to repeat period and count page page when repeat = YES has not changed but repeat count is not set', async () => {
      req.session.createAppointmentJourney.repeat = YesNo.YES
      req.session.createAppointmentJourney.repeatPeriod = AppointmentRepeatPeriod.FORTNIGHTLY
    ...
    it('should redirect to check answers page when repeat = YES has not changed and repeat period and count are set', async () => {
      req.session.createAppointmentJourney.repeat = YesNo.YES
      req.session.createAppointmentJourney.repeatPeriod = AppointmentRepeatPeriod.DAILY
      req.session.createAppointmentJourney.repeatCount = 7
      req.body = {
        repeat: YesNo.YES,
      }
