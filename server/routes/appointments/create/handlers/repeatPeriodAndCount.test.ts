...
describe('Route Handlers - Create Appointment - Repeat Period and Count', () => {
  ...
  describe('GET', () => {
    it('should render the repeat period and count view', async () => {
      ...
      expect(res.render).toHaveBeenCalledWith('pages/appointments/create/repeat-period-and-count')
    })
  })
  ...
  describe('POST', () => {
    it('should save repeat = YES, period and count in session and redirect to check answers page', async () => {
      ...
      expect(req.session.createAppointmentJourney.repeat).toBeUndefined()

      await handler.POST(req, res)

      expect(req.session.createAppointmentJourney.repeat).toEqual(YesNo.YES)
      expect(req.session.createAppointmentJourney.repeatPeriod).toEqual(AppointmentRepeatPeriod.WEEKLY)
      expect(req.session.createAppointmentJourney.repeatCount).toEqual(6)
