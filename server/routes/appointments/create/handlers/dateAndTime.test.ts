...
describe('Route Handlers - Create Appointment - Date and Time', () => {
  ...
  describe('GET', () => {
    it('should render the date and time view', async () => {
      ...
      expect(res.render).toHaveBeenCalledWith('pages/appointments/create/date-and-time')
    })
  })
  ...
  describe('POST', () => {
    it('should save start date, start time and end time in session and redirect to repeat page', async () => {
      ...
      expect(req.session.createAppointmentJourney.startDate).toEqual({
        day: startDate.day,
        month: startDate.month,
        year: startDate.year,
        date: req.body.startDate.toRichDate(),
      })
      expect(req.session.createAppointmentJourney.startTime).toEqual({
        hour: 11,
        minute: 30,
        date: req.body.startTime.toDate(tomorrow),
      })
      expect(req.session.createAppointmentJourney.endTime).toEqual({
        hour: 13,
        minute: 0,
