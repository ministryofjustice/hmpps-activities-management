...
export default function Create({
  ...
  const post = (path: string, handler: RequestHandler, type?: new () => object, stepRequiresJourney = false) =>
    router.post(
      path,
      setUpJourneyData(tokenStore),
      emptyAppointmentJourneyHandler(stepRequiresJourney),
  ...
  const checkAnswersRoutes = new CheckAnswersRoutes(activitiesService, metricsService)
  ...
  get('/start-prisoner/:prisonNumber/name', startJourneyRoutes.PRISONER_FROM_CONFIRMATION)
