...
const view = fs.readFileSync('server/views/pages/activities/exclusions/select-prisoner.njk')

describe('Views - Activity Management - Select Prisoner to change schedule', () => {
  ...
  it('should display prisoner list with radios and "Select prisoner and continue" button when prisoners are in context', () => {
    ...
    expect($('#prisoner-results-text').text().trim()).toEqual(
      'Select the correct person from the list (maximum 50 results shown)',
    )
  ...
  it('should display single prisoner without radios when single prisoner is found', () => {
    ...
    expect($('#prisoner-results-text').text().trim()).toEqual('There is 1 matching person')
