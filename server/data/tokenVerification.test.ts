...
const userWithToken = { token: 'user-token', username: 'jbloggs', authSource: 'nomis' }
  ...
  describe('POST requests', () => {
    ...
    describe('Token Verification enabled', () => {
      ...
      it('Calls verify and parses response', async () => {
        fakeApi.post('/token/verify').matchHeader('authorization', 'Bearer user-token').reply(200, { active: true })
        const data = await verifyToken({ user: userWithToken, verified: false } as Request)
        expect(data).toEqual(true)
