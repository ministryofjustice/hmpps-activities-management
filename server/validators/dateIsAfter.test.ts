...
describe('dateIsAfter', () => {
  class DummyForm {
    @Expose()
    @DateIsAfter(toDate('2022-12-22'), { message: "Enter date after today's date" })
    date: SimpleDate
  }
  ...
  it('should fail validation for a date before the supplied date', async () => {
    ...
    expect(errors).toEqual([{ property: 'date', error: "Enter date after today's date" }])
  })

  it('should fail validation for a date equal to the supplied date', async () => {
    const body = {
      date: plainToInstance(SimpleDate, {
        day: 22,
        month: 12,
