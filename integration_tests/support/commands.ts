...
import rolloutPlanEAEnabled from '../fixtures/activitiesApi/rolloutEAEnabled.json'
...
Cypress.Commands.add('signInEAEnabled', (options = { failOnStatusCode: true }) => {
  cy.request('/')
  cy.stubEndpoint('GET', '/rollout/MDI', rolloutPlanEAEnabled)
  return cy.task('getSignInUrl').then((url: string) => cy.visit(url, options))
})
